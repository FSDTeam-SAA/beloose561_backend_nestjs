...
describe('Inventory bulk preview', () => {
  ...
  it('preserves CSV barcodes and maps retailer columns', () => {
    const result = service.previewBulkInventory(
      csv('Barcode,Qty,Price\n0012345,12,13'),
      { mapping },
    );
    expect(result.totalRows).toBe(1);
    expect(result.mappedPreview).toEqual([
      { upc: '0012345', quantity: '12', price: '13' },
    ]);
  });
  ...
  it('preserves formatted Excel barcodes', () => {
    ...
    expect(
      service.previewBulkInventory(file, { mapping }).mappedPreview?.[0].upc,
    ).toBe('000123');
  });
  ...
  it('limits previews to ten rows while reporting the total', () => {
    ...
    const mapped = service.previewBulkInventory(file, { mapping });
    expect(mapped.mappedRows).toHaveLength(12);
    expect(mapped.mappedRows?.[11]).toEqual({
      upc: '001',
      quantity: '1',
      price: '2',
    });
  ...
  it('rejects ambiguous headers and invalid mappings', () => {
    ...
    expect(() =>
      service.previewBulkInventory(file, { mapping: { Wrong: 'upc' } }),
    ).toThrow('Unknown column');
    expect(() =>
      service.previewBulkInventory(file, { mapping: { Barcode: 'upc' } }),
    ).toThrow('must include');
    expect(() =>
      service.previewBulkInventory(file, {
        mapping: { Barcode: 'upc', Qty: 'upc' },
      }),
    ).toThrow('only once');
  });
  ...
  it('rejects empty files and excessive row counts', () => {
    expect(() =>
      service.previewBulkInventory(csv('Barcode,Qty,Price'), {}),
    ).toThrow('data rows');
    expect(() =>
      service.previewBulkInventory(
